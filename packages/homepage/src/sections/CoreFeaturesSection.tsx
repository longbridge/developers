...
export function CoreFeaturesSection({ locale }: CoreFeaturesSectionProps) {
  const content = LOCALE[locale] ?? LOCALE.en
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: CORE_CSS }} />
      <section data-lbus-component="core-features" className="core-section">
        <div className="core-header">
          <h2 className="core-title">{content.title}</h2>
          <p className="core-subtitle">{content.subtitle}</p>
        </div>
            ...
            return (
              <a
                ...
                style={
                  {
                    '--card-accent': accent,
                    background: `radial-gradient(280px 140px at 100% 0%, color-mix(in srgb, ${accent} 9%, transparent), transparent 70%), var(--vp-c-bg)`,
              ...
              >
                <div className="core-card-inner">
                  <div className="core-card-top">
                    ...
                    />
                    <span className="core-card-title">{content.products[key]?.title}</span>
                  </div>
                  <p className="core-card-desc">{content.products[key]?.desc}</p>

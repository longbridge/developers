...
const LOCALE = {
  ...
  'zh-CN': {
    title: '几分钟内开始构建',
    subtitle: '配置环境、完成认证、发起第一个 API 调用 — 从零到获取实时数据所需的一切。',
    cards: {
      auth: { title: '认证配置', desc: '注册 OAuth 2.0 客户端，获取凭证，配置 SDK 自动令牌管理。', cta: '配置指南' },
      api: {
        title: 'API 参考',
        desc: '浏览 100+ 行情、交易、持仓和内容端点。直接在浏览器中尝试请求。',
        cta: '探索 API',
      },
      cli: {
        title: '安装 CLI',
        desc: 'macOS、Linux 和 Windows 一行命令安装。130+ 命令，交互式 TUI 和 JSON 输出。',
        cta: '立即安装',
      },
    } as Record<string, { title: string; desc: string; cta: string }>,
  },
  'zh-HK': {
    title: '幾分鐘內開始構建',
    subtitle: '配置環境、完成認證、發起第一個 API 調用 — 從零到獲取即時數據所需的一切。',
    cards: {
      auth: { title: '認證配置', desc: '註冊 OAuth 2.0 客戶端，獲取憑證，配置 SDK 自動令牌管理。', cta: '配置指南' },
      api: {
        title: 'API 參考',
        desc: '瀏覽 100+ 行情、交易、持倉和內容端點。直接在瀏覽器中嘗試請求。',
        cta: '探索 API',
      },
      cli: {
        title: '安裝 CLI',
        desc: 'macOS、Linux 和 Windows 一行命令安裝。130+ 命令，互動式 TUI 和 JSON 輸出。',
        cta: '立即安裝',
      },
    } as Record<string, { title: string; desc: string; cta: string }>,
  },
...
.gs-card-icon-wrap.dark-mode {
  background: var(--gs-icon-bg-dark);
}
.gs-card-icon {
  display: flex;
  flex-shrink: 0;
...
export function GetStarted({ locale }: GetStartedProps) {
  const content = LOCALE[locale] ?? LOCALE.en
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: GS_CSS }} />
      <section data-lbus-component="get-started" className="gs-section">
        <div className="gs-bg" />
        <div className="gs-content">
          <h2 className="gs-title">{content.title}</h2>
          <p className="gs-subtitle">{content.subtitle}</p>
          <div className="gs-cards">
            {entries.map((e) => (
              <a key={e.key} href={e.href} className="gs-card">
                <div className="gs-card-header">
                  <div
                    className="gs-card-icon-wrap"
                    style={
                      {
                        '--gs-icon-color': e.iconColor,
                        '--gs-icon-bg': e.iconBg,
                        '--gs-icon-bg-dark': e.iconBgDark,
                        background: e.iconBg,
                  ...
                  >
                    <span
                      ...
                      style={{ color: e.iconColor } as React.CSSProperties}
                      dangerouslySetInnerHTML={{ __html: e.icon }}
                    />
                  </div>
                  <h3 className="gs-card-title">{content.cards[e.key]?.title}</h3>
                </div>
                <p className="gs-card-desc">{content.cards[e.key]?.desc}</p>
                <span className="gs-card-link">
                  {content.cards[e.key]?.cta}
                  <svg
                    width="14"
                    height="14"
                    viewBox="0 0 24 24"

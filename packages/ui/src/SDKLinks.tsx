...
// Utility
// ──────────────────────────────────────────────
const snakeCase = (str: string) => str.replace(/[A-Z]/g, (l) => `_${l.toLowerCase()}`)
const camelCase = (str: string) => str.replace(/_([a-z])/g, (_, l) => l.toUpperCase())
const capitalize = (str: string) => str.charAt(0).toUpperCase() + str.slice(1)

// ──────────────────────────────────────────────
// C++ Doxygen anchor map
// ──────────────────────────────────────────────
...
const JAVA_ANCHOR_MAP: Record<string, string> = {
  ...
  getCandlesticks:
    'getCandlesticks(java.lang.String,com.longbridge.quote.Period,int,com.longbridge.quote.AdjustType,com.longbridge.quote.TradeSessions)',
  ...
  getHistoryCandlesticksByOffset:
    'getHistoryCandlesticksByOffset(java.lang.String,com.longbridge.quote.Period,com.longbridge.quote.AdjustType,boolean,java.time.LocalDateTime,int,com.longbridge.quote.TradeSessions)',
  getHistoryCandlesticksByDate:
    'getHistoryCandlesticksByDate(java.lang.String,com.longbridge.quote.Period,com.longbridge.quote.AdjustType,java.time.LocalDate,java.time.LocalDate,com.longbridge.quote.TradeSessions)',
  getHistoryMarketTemperature:
    'getHistoryMarketTemperature(com.longbridge.Market,java.time.LocalDate,java.time.LocalDate)',
  ...
  getEstimateMaxPurchaseQuantity:
    'getEstimateMaxPurchaseQuantity(com.longbridge.trade.EstimateMaxPurchaseQuantityOptions)',
}
...
export function SDKLinks({
  ...
  const cppClassSlug = snakeCase(effectiveKlass).replace(/^_/, '')
  const cppBase = `https://longbridge.github.io/openapi/cpp/classlongbridge_1_1${module}_1_1_${cppClassSlug}.html`
  const cppAnchor = CPP_ANCHOR_MAP[`${module}:${method}`]
  const cppUrl = cppAnchor ? `${cppBase}#${cppAnchor}` : cppBase
  ...
  const baseLinks = [
    {
      title: 'Python',
      color: '#3572a5',
      label: `longbridge.openapi.${effectiveKlass}.${snakeMethod}`,
      url: `https://longbridge.github.io/openapi/python/reference_all/#longbridge.openapi.${effectiveKlass}.${snakeMethod}`,
    },
    {
      title: 'Rust',
      color: '#dea584',
      label: `longbridge::${module}::${effectiveKlass}#${snakeMethod}`,
      url: `https://longbridge.github.io/openapi/rust/longbridge/${module}/struct.${effectiveKlass}.html#method.${snakeMethod}`,
    },
    {
      title: 'Go',
      color: '#00ADD8',
      label: `${effectiveKlass}.${methodGo}`,
      url: `https://pkg.go.dev/github.com/longbridge/openapi-go/${module}#${effectiveKlass}.${methodGo}`,
    },
    {
      title: 'Node.js',
      color: '#f1e05a',
      label: `${effectiveKlass}#${camelMethod}`,
      url: `https://longbridge.github.io/openapi/nodejs/classes/${effectiveKlass}.html#${camelMethod.toLowerCase()}`,
    },
    {
      title: 'Java',
      color: '#b07219',
      label: `${effectiveKlass}.${methodJava}`,
      url: `https://longbridge.github.io/openapi/java/com/longbridge/${module}/${effectiveKlass}.html#${javaAnchor}`,
    },
    {
      title: 'C++',
      color: '#f34b7d',
  ...
  return (
    <div data-lbus-component="sdk-links">
      {titleStr && level === 2 && <h2>{titleStr}</h2>}
      {titleStr && level === 3 && <h3>{titleStr}</h3>}
      {titleStr && level === 4 && <h4>{titleStr}</h4>}
      {titleStr && level === 5 && <h5>{titleStr}</h5>}
          ...
          {links.map((link) => (
            <tr key={link.title}>
              <td
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', color: 'var(--vp-c-text-1)' }}>
                  <span
                    style={{
              ...
              <td
                ...
                <a
                  ...
                  style={{ color: 'var(--vp-c-brand-1)', textDecoration: 'none' }}>
                  {link.label}
                </a>
              </td>
            </tr>

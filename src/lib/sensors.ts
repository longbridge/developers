/**
 * Sensors (神策) analytics wrapper — ported 1:1 from the legacy VitePress theme
 * (docs/.vitepress/theme/utils/sensors.ts).
 *
 * The SDK script is loaded via <script defer> in BaseLayout.astro
 ...
 * based on the build-time proxy (`window.__LB_PROXY__`), and exposes a queued
 * `track()` / `login()` API so callers don't need to await SDK readiness.
 *
 * Default behaviour (once init runs):
 * - `$pageview` fires on first load and on every SPA route change
 *   (`is_track_single_page: true`).
 * - Every click is auto-reported as `$WebClick` (`heatmap.clickmap: 'default'`).
 * - Page stay / scroll depth reported as `$WebStay`.
 * - If signed in (`window.longportInternal.isLogin()`), the member id is bound
 *   via `sensors.login()` after init.
 */
...
/** Build-time proxy: 'canary' | 'production' (window.__LB_PROXY__). */
...
class Sensors {
  ...
  private getServerUrl(): string {
    // window.__LB_PROXY__ is set by the BaseLayout head inline script from
    // process.env.PROXY (same source as oneTapProxy). 'canary' => default
    // reporting bucket; anything else => production.
    const proxy: LbProxy =
      typeof window !== 'undefined' ? (window.__LB_PROXY__ ?? 'production') : 'production'

...
Temporarily disable a price alert without deleting it.

<CliCommand>
longbridge alert disable 486469
</CliCommand>

<SDKLinks module="alert" klass="AlertContext" method="disable" />
...
<tr><td className="http-basic-key">HTTP URL</td><td>/v1/account/alerts/{alert_id}/disable</td></tr>
</tbody>
</table>
...
| ---- | ---- | -------- | ----------- |
| alert_id | string | YES | Alert ID to disable |
...
func main() {
	...
	defer c.Close()
	if err := c.Disable(context.Background(), "486469"); err != nil {
		log.Fatal(err)
	}
}
```

...
Re-enable a previously disabled price alert.
...
<tr><td className="http-basic-key">HTTP URL</td><td>/v1/account/alerts/{alert_id}/enable</td></tr>
</tbody>
</table>
...
| ---- | ---- | -------- | ----------- |
| alert_id | string | YES | Alert ID to enable |
...
func main() {
	...
	defer c.Close()
	if err := c.Enable(context.Background(), "486469"); err != nil {
		log.Fatal(err)
	}
}
```

...
Get the hierarchical sub-sector tree for an industry group, including stock count, daily change, and YTD change at each node. Symbols come from `industry_rank`.

<CliCommand>
longbridge industry-peers IN00258.US
longbridge industry-peers IN20337.HK
</CliCommand>
...
| ---- | ---- | -------- | ----------- |
| symbol | string | YES | Industry unique identifier (BK/MARKET/ID format) from `industry_rank` |
...
resp = ctx.industry_peers("IN00258.US", "US")
print(resp)
```
...
async def main() -> None:
    ...
    resp = await ctx.industry_peers("IN00258.US", "US")
    print(resp)
...
async function main() {
  ...
  const resp = await ctx.industryPeers('IN00258.US', 'US')
  console.log(resp)
}
...
class Main {
    public static void main(String[] args) throws Exception {
        try (OAuth oauth = new OAuthBuilder("your-client-id").build(url -> System.out.println("Open to authorize: " + url)).get();
            ...
            var resp = ctx.getIndustryPeers("IN00258.US", "US").get();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let resp = ctx.industry_peers("IN00258.US", "US").await?;
    println!("{:?}", resp);
    Ok(())
}
```
...
int main() {
    OAuthBuilder("your-client-id").build(
        ...
        [](auto res) {
            ...
            FundamentalContext ctx = FundamentalContext::create(config);
            ctx.industry_peers("IN00258.US", "US", [](auto resp) {
...
func main() {
	...
	defer c.Close()
	resp, err := c.IndustryPeers(context.Background(), "IN00258.US", "US")
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("%+v\n", resp)
...
| symbol | string | false | Sector symbol (present on root node; empty string on child nodes) |

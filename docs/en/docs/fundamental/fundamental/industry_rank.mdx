...
Get the industry ranking list by market and indicator. The returned Symbol can be passed directly to `industry_peers` to explore the sub-sector hierarchy.
...
| ∟ ∟ symbol | string | false | Industry symbol (`BK/MARKET/ID` format), usable in `industry_peers` |

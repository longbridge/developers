...
Cancel a grid order. The grid stops immediately and no further trigger orders are placed. Cancellation is permanent — a canceled grid cannot be restarted; submit a new grid order instead.

<CliCommand>
# Cancel a grid order by ID
longbridge grid cancel 764609681686573056
</CliCommand>

<SDKLinks module="grid" klass="GridContext" method="cancel" hideGo />
...
<tr><td className="http-basic-key">HTTP URL</td><td>/v1/gridtrading/cancel</td></tr>
</tbody>
</table>
...
resp = ctx.cancel("764609681686573056")
print(resp)
```
...
async function main() {
  ...
  const resp = await ctx.cancel('764609681686573056')
  console.log(resp)
}
...
class Main {
    public static void main(String[] args) throws Exception {
        try (OAuth oauth = new OAuthBuilder("your-client-id").build(url -> System.out.println("Open to authorize: " + url)).get();
            ...
            ctx.cancel("764609681686573056").get();
        }
    }
}
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ctx.cancel("764609681686573056").await?;
    Ok(())
}
```
...
run(const OAuth& oauth)
{
    ...
    ctx.cancel("764609681686573056", [](auto res) {
        if (!res) {
...
| ------ | ------------------------------------------------------------ | ------ |
| 200    | The grid order was canceled successfully.                    | None   |

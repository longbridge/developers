...
Restart a suspended grid order. The grid resumes from its saved configuration and begins triggering orders again. Only a grid previously paused with [Suspend Grid Order](./suspend) can be restarted.
...
<SDKLinks module="grid" klass="GridContext" method="restart" hideGo />
...
<tr><td className="http-basic-key">HTTP URL</td><td>/v1/gridtrading/restart</td></tr>
</tbody>
</table>
...
resp = ctx.restart("764609681686573056")
print(resp)
```
...
async function main() {
  ...
  const resp = await ctx.restart('764609681686573056')
  console.log(resp)
}
...
class Main {
    public static void main(String[] args) throws Exception {
        try (OAuth oauth = new OAuthBuilder("your-client-id").build(url -> System.out.println("Open to authorize: " + url)).get();
            ...
            ctx.restart("764609681686573056").get();
        }
    }
}
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ctx.restart("764609681686573056").await?;
    Ok(())
}
```
...
run(const OAuth& oauth)
{
    ...
    ctx.restart("764609681686573056", [](auto res) {
        if (!res) {

...
<tr><td className="http-basic-key">HTTP URL</td><td>/v1/fundamental/institution_rating_detail</td></tr>
</tbody>
</table>
...
| Name | Type | Required | Description |
| ---- | ---- | -------- | ----------- |
| list | object[] | 是 | 評級詳情列表 |
| list[].symbol | string | 是 | 證券代碼 |
| list[].broker | string | 否 | 券商名稱 |
| list[].analyst | string | 否 | 分析師姓名 |
| list[].rating | string | 否 | 評級（如 Buy、Hold、Sell） |
| list[].target_price | double | 否 | 目標價 |
| list[].date | string | 否 | 評級日期（YYYY-MM-DD） |

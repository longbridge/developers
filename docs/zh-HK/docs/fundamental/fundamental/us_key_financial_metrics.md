...
async function main() {
  ...
  const resp = await ctx.usKeyFinancialMetrics("AAPL.US", report="annual")
  console.log(resp)
}
...
class Main {
    public static void main(String[] args) throws Exception {
        try (OAuth oauth = new OAuthBuilder("your-client-id").build(url -> System.out.println("Open to authorize: " + url)).get();
            ...
            var resp = ctx.getUsKeyFinancialMetrics("AAPL.US", report="annual").get();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let resp = ctx.us_key_financial_metrics("AAPL.US", report="annual").await?;
    println!("{:?}", resp);
    Ok(())
}
```

...
`longbridge acp` 將 [LongbridgeAI](https://longbridge.com/ai) 作為兼容 [Agent Client Protocol](https://agentclientprotocol.com/)（ACP）的 Agent 運行。你可以從任何支持啟動自定義 ACP Agent 的客戶端應用中與 LongbridgeAI 對話，查詢實時行情、分析公司基本面、解讀賬戶持倉。

## 使用場景

- 在常用 AI 客戶端中直接使用 LongbridgeAI，無需切換應用
...
保存後新建 External Agent 對話並選擇 **LongbridgeAI**。詳見 [Zed External Agents 文檔](https://zed.dev/docs/ai/external-agents)。

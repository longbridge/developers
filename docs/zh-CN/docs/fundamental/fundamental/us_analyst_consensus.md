...
async def main() -> None:
    ...
    resp = await ctx.us_analyst_consensus("AAPL.US", report="annual")
    print(resp)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let resp = ctx.us_analyst_consensus("AAPL.US", report="annual").await?;
    println!("{:?}", resp);
    Ok(())
}
```
...
返回结构灵活的 JSON 对象，包含分析师一致预期数据（营收和 EPS 预测、目标价分布、买入/持有/卖出评级分布）。具体字段因数据可用性而异。可通过 CLI 使用 `--format json` 或打印原始响应查看数据结构。
